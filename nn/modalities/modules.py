...
from nn.utils import RotaryEmbedding
...
class BlockEncoder(nn.Module):
    ...
    def forward(self, x, padding_mask, alibi_bias, alibi_scale): # dummy_data
        # if not dummy_data:
...
class AltBlock(nn.Module):
    ...
    def forward(self, x, padding_mask=None, alibi_bias=None, rotation_freqs=None):
        if self.layer_norm_first:
            ...
            x = x + self.drop_path(self.attn(x, padding_mask, alibi_bias, rotation_freqs))
            r = x = self.norm1(x)
            x = self.mlp(x)
            t = x
    ...
    def forward(self, x, padding_mask=None, alibi_bias=None, rotation_freqs=None):
        ...
        if self.cosine_attention:
            ...
            if self.rotary_emb is not None:
                k = self.rotary_emb.apply_rotation(k, rotation_freqs)
                q = self.rotary_emb.apply_rotation(q, rotation_freqs)
        ...
        # padding mask removes qk dot products on masked positions, softmax after sets to 0
