...
class ModalitySpecificEncoder(nn.Module):
    ...
    def contextualized_features(
        ...
        if self.fixed_positional_encoder is not None:
            x = x + self.fixed_positional_encoder(x, sample_rates)

        # if self.dummy_data:
        #     x = (torch.arange(0, 100)).unsqueeze(0).unsqueeze(-1).expand(-1, -1, 768).to(x)

        if mask:
            if clone_batch > 1:
                # logger.info("\n\n pre repeat interleave x.size(): {}".format(x.size()))  # torch.Size([12, 200, 768])
                x = x.repeat_interleave(clone_batch, 0)
                sample_rates = sample_rates.repeat_interleave(clone_batch)
        ...
        if mask and remove_masked:
            ...
            if padding_mask is not None and padding_mask.any():
                ...
                # print(f"total number of unmasked positions: {x.size(1)}")
                # print(f"padded positions in input: {padding_mask.sum(dim=1)} "
                #       f"ratio: {torch.div(padding_mask.sum(dim=1), padding_mask.size(1))}")
                # print(f"padded positions in unmasked input: {masked_padding_mask.sum(dim=1)} "
                #       f"ratio: {torch.div(masked_padding_mask.sum(dim=1), masked_padding_mask.size(1))}")
        ...
        if precomputed_mask is not None:
            ...
            if mask_prob > 0:
                if cfg.mask_length == 1:
                    ...
                    # applies masking to padded regions too. This ensures equal padding for variable length signals
                    # if padding mask is provided, masked regions stay fully unmasked, ending up in the unmasked input
                    # to the student. This means that the unpadded, shorter portion of an input is masked with the same
                    # ratio as the longer input, but all padded positions are added, leaving much less real signal
                    mask_padding = True

                    mask = compute_mask_indices(
                        (B, T),
                        padding_mask if not mask_padding else None,
                        mask_prob,
                        cfg.mask_length,
                        min_masks=1,
                    ...
                    # if self.dummy_data:
                    #     trues = np.full((1, 25), True)
                    #     falses = np.full((1, 25), False)
                    #     mask = np.concatenate((trues, falses, trues, falses), axis=1)
    ...
    def make_maskinfo(self, x, mask, shape=None):
        ...
        # all unmasked positions (keep) first, then masked positions. They are in a shuffled order due to unstable torch
        ids_shuffle = mask.argsort(dim=1)

        # inverse permutation of ids_shuffle. To reassamble the full sequence
