...
from nn.utils import RotarySampleRateEmbedding
...
class Data2VecMultiModel(BaseFairseqModel, FusedSegmentationMixin):
    ...
    def __init__(self, cfg: Data2VecMultiConfig, modalities, skip_ema=False, task=None):
        ...
        self.task = task
        self.positional_encoder_type = cfg.positional_encoder_type
        self.max_sample_size = cfg.max_sample_size
        # self.dummy_data = cfg.dummy_data
        self.feature_enc_layers = eval(cfg.conv_feature_layers)
        self.head_dim = cfg.embed_dim // cfg.num_heads

        # calculate maximum allowed length after feature encoder
        max_positions = [self.max_sample_size]
        for xx in self.feature_enc_layers:
            max_positions = [get_conv_size(max_positions, [min(10, xx[1])], [0], [1], [xx[2]], dim=1)[0]]

        if self.positional_encoder_type == "rotary":
            self.rotary_emb = RotarySampleRateEmbedding(dim=self.head_dim, max_seq_len=max_positions[0])
        else:
    ...
    def forward(
        ...
        x = extractor_out["x"]
        batch_srs = extractor_out["sample_rates"]
        device, dtype, seq_len = x.device, x.dtype, x.shape[-2]
        encoder_mask = extractor_out["encoder_mask"]

        if self.positional_encoder_type == "rotary":

            if not features_only or force_remove_masked:
                pos_map = encoder_mask.ids_keep
            else:
                pos_map = None

            freqs = self.rotary_emb.get_rotation_frequencies(batch_srs, pos_map, device, dtype, seq_len)
        ...
        for i, blk in enumerate(self.blocks):
            if (
                    ...
                    # don't drop all layers in a forward pass
                    or (i + 1 == self.cfg.depth and not layer_results)
            ):
        ...
        with torch.no_grad():
            ...
            ema_srs = ema_input["sample_rates"]

            # if self.dummy_data:
            #     ema_srs = torch.LongTensor([8000])

            ema_alibi_bias = ema_input.get("alibi_bias", None)
            ema_alibi_scale = ema_input.get("alibi_scale", None)
            ema_input = ema_input["x"]
            device, dtype, seq_len = ema_input.device, ema_input.dtype, ema_input.shape[-2]

            if self.positional_encoder_type == "rotary":
                freqs = self.rotary_emb.get_rotation_frequencies(ema_srs, None, device, dtype, seq_len)
            else:
                freqs = None
    ...
    def extract_features(
            self, source, mode=None, padding_mask=None, mask=False, remove_extra_tokens=True, sample_rates=None
    ):
        res = self.forward(
            source,
