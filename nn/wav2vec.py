...
class Wav2Vec2CcasFinetuneConfig(Wav2Vec2CtcConfig):
    ...
    event_detection: bool = II("criterion.event_detection")
...
class Wav2VecEncoderModOut(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2CcasFinetuneConfig, output_size=None):
        ...
        arg_overrides = {
            ...
            "event_detection": cfg.event_detection
        }
        ...
        targ_d = None
        self.proj = None
        self.event_detection = cfg.event_detection
    ...
    def forward(self, source, padding_mask=None, target=None, sample_rates=None, **kwargs):
        # we use BC Learning:
        ...
        if not self.event_detection:    # sample-level predictions of each class

            # remove initially padded tokens so they don't influence the max-pooling
            if padding_mask is not None and padding_mask.any():
                expanded_padding_mask = padding_mask.unsqueeze(-1).expand_as(x)
                x.masked_fill_(expanded_padding_mask, -torch.inf)

            x, _ = torch.max(x, dim=1)  # Output shape is BC
            target, _ = torch.max(target.type(torch.int64), dim=1)  # Output shape is BC
