...
class FileAudioLabelDataset(RawAudioDataset, ABC):
    ...
    audio2label_re = re.compile(r"(?P<pre>.*)(?P<dir>wav)(?P<post>/.*\.)(?P<ext>[a-z0-9]+)$", re.IGNORECASE)
    ...
    def postprocess(self, feats, curr_sample_rate):
        ...
        # disable exception from fairseq code
    ...
    def __getitem__(self, index):
        ...
        # pass along sample rate for positional encoding
        data_dict = {"id": index, "source": feats, "sr": curr_sample_rate}
        ...
        if self.return_labels:
            ...
            token_length = round(float(wav_len / ft_out_size))
            ...
            if len(start_frame_lbl) > 0 and len(end_frame_lbl) > 0 and len(lbl_cat) > 0:
                for ii, (s, e, l) in enumerate(zip(start_frame_lbl, end_frame_lbl, lbl_cat)):
                    ...
                    # check if label overlaps with any token start, so it is not missed in the interpolation step
                    lbl_length = e - s
                    if lbl_length < token_length:
                        k0 = np.searchsorted(target_vector_idx_vector, s, side='left')
                        k1 = np.searchsorted(target_vector_idx_vector, e, side='right') - 1
                        if k0 > k1:
                            # print("Short label would have been skipped!")
                            prev_idx = max(0, np.searchsorted(target_vector_idx_vector, s, side='right') - 1)
                            s = int(target_vector_idx_vector[prev_idx])

                    if self.use_focal_loss:
                        source_vector[s:e+1, l] = 1
                        if self.do_focal_prediction and self.unique_labels[-1].lower() == "focal":
                            # is it a focal call or not
                            focal_lbl = foc[ii]
                            if focal_lbl == 1:
                                source_vector[s:e+1, -1] = focal_lbl
                    else:
                        source_vector[s:e+1] = l + 1
    ...
    def collate_samples(self, samples):
        ...
        if samples[0].dim() > 1:    # empty tensor for padding
        ...
        padding_mask = (torch.BoolTensor(
            collated_samples.shape).fill_(False) if self.pad else None)     # fill with false

        for i, (source, size) in enumerate(zip(samples, sizes)):
            diff = size - target_size
            if diff == 0:
                collated_samples[i] = source    # put whole sample into the zero tensor
            elif diff < 0:
                assert self.pad
                if source.dim() > 1:
                    targets = [torch.cat([x, x.new_full((-diff,), 0.0)]) for x in source.t()]
                    collated_samples[i] = torch.cat(targets, dim=0).view(-1, target_size).t()
                else:
                    collated_samples[i] = torch.cat([source, source.new_full((-diff,), 0.0)])   # sample + 0 pads
    ...
    def collater(self, samples):
        ...
        out = {"id": torch.LongTensor([s["id"] for s in samples]),
               "sample_rate": torch.LongTensor([s["sr"] for s in samples])}
